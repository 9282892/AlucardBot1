	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola:) @${num.split('@')[0]}\nSeja bem vindo ao grupo siga as regras para no levar ban *${mdata.subject}*`
				...
				teks = `Saiu por que no aguentou os 3cm do adm @${num.split('@')[0]}ðŸ‘‹`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '😎Pronto krl😎🤙',
				error: {
					stick: '⚠️ Fudeu kleitom teve uma falha⚠️',
					Iv: '⚠️Link não válido⚠️'
				},
				only: {
					group: '⚠️Este comando só pode ser usado em grupos carai!⚠️',
					ownerG: '⚠️Este comando só pode ser usado pelos adms do grupo seu membro comum⚠️',
...
case 'yt2mp3':
				...
				case 'info':
					...
					break
case 'ytmp3':
					if (args.length < 1) return reply('Onde está o url seu acefalo?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
...
case 'grup':
					...
					if (args[0] === 'abrir') {
					    reply(`grupo aberto para membros comuns`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`grupo fechado para os membros comuns`)
...
case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply('Cade o txt krl?\nExemplo: ${prefix}harrypotter Dkofc')
...
case 'infonomor':
               client.updatePresence(from, Presence.composing) 
                ...
                break
case 'ytmp4':
					if (args.length < 1) return reply('Cade o url seu jegue?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
...
case 'animehug':
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'vagabundo expulso com sucesso :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
...
case 'textodeneve':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo boas vindas ativado com sucesso nesta porra')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Modo boas vindas desativado com sucesso krlâœ”ï¸')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
                                      break
...
if (args.length < 1) return reply('TrashфDkofcꪶ Número do meu dono😎🤙:WA.me/558494740630\n😎🤙Youtube do meu \ndono:https://youtube.com/channel/UCOqlFzRrB8IZUWUx1cwCeUw\n \nNúmero da cremosa do meu dono que ajudou a fazer o menu 😎🤙ঔৣ☬✞𝐓𝐫𝐚𝐬𝐡|𝐉𝐮𝐰 ✞☬ঔৣ: wa.me/558491093889')
                                        break
case 'porno':
if (args.length < 1) return reply('para de ver porno garai')
break
case 'bomdia':
if (args.length < 1) return reply('Bom dia meu amigo')
break
case 'boatarde':
if (args.length < 1) return reply('Boa tarde só prós do verdade😎🤙')
break
case 'boanoite':
if (args.length < 1) return reply('Boa noite, ta na hora de dormir seu baiano')
break
case 'timer':
